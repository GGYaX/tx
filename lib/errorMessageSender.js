...
function emailEntry(errorDesc, emailConf, callback) {
	...
	this.mailText = function() {
		...
		for(var i = 0; i < that.message.length; i ++) {
			that.text += '\n' + that.message[i]['desc'] + ' -> OriginData is: ' + that.message[i]['originData'] + ' On file"' + that.message[i]['file'] +'"';
		}
	...
	this.send = function() {
	  ...
	  transport.sendMail(mailOptions, function(error, response) {
	  	if(error){
	  		...
	  		console.log("[OK] Message sent: " + response.message + '\n');
	  	}
...
emailEntry.prototype.addMessage = function(errorCode, data, file) {
	this.message.push({'errorCode': errorCode, 'originData': data, 'desc': this.errorDesc[errorCode], 'file':file});
};
