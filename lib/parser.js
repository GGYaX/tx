...
function dataParser (string, serie, separator, tablePrefix, callback) {
	if(string.lastIndexOf(separator) === -1) {
		...
		switch(parseInt(serie)) {
			case 1000: {
				// call 1000 parser
				var data1000 = new dataGisol();
				// var arrayDesc1000 = ['time', 'record', 'direct', 'temp1', 'globale', 'temp2', 'diffus', 'temp3', 'infra-rouge', 'temp4'];
				// var arrayUnit1000 = ['TS', 'RN', 'mV', 'mV', 'mV', 'mV', 'mV', 'mV', 'mV', 'mV'];
				// var arrayType1000 = ["","","Avg","Avg","Avg","Avg","Avg","Avg","Avg","Avg"];
				...
				stringSplitted.forEach(function (r) {
					...
					if(r === '' || /^ /i.test(r)) {
						...
						data1000.set(arrayDesc[i], value);
						i++;
					}
				});
				callback(undefined, data1000.data, string);
			} break;
			case 1000: {
				// call 1000 parser
			} break;
...
// dataParser('"2012-04-05 11:12:00",212975,0,0,1.648,0,1.895,0,-0.247,0', 1000, ',');
