...
function fileParser(filename, serie, position) {
	...
	this.needtoWriteInto = function() {
		...
		for(var i = 0; i < globalConf.fileReader.files.length; i++) {
			...
			if(i === globalConf.fileReader.files.length - 1) {
				globalConf.fileReader.files.push({
					'name': path.basename(filename),
					'readFrom': that.lineReaded
				});
			}
		}
		...
		that.errorParser.forEach(function(error) {
			if(that.nbtestParser !== 0 && error.length >= globalConf['acceptErreurRatio'] * ((that.nbtestParser >= globalConf['checkErreurInterval']) ? globalConf['checkErreurInterval'] : that.nbtestParser)) {  // pour erreur de parser
				...
				for(var i = 0; i < error.length; i ++) {
					sender.addMessage(error[i]['code'], error[i]['originData'], that.filename);
				}
	...
	if(fs.existsSync(this.filename)) {
		...
		new lazy(stream)
			...
			.forEach(function(line) {
				...
				// console.log(line);
				dataParser(line.toString(), that.serie, ',', globalConf['tablePrefix'][that.serie], function(err, data, originData) {
					that.nbtestParser ++;
					if(err) {
						console.log(err);
						that.errorParser[that.whichInterval].push(err);
					} else {
						dataTransformer(data, that.serie);
